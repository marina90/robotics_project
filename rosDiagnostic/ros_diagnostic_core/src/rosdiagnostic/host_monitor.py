...
from ros_diagnostic_msgs.msg import cpuload
...
class HostMonitor(object):
	...
	diagnostic.hostname = self._hostname
        diagnostic.ipaddress = self._ipaddress
        diagnostic.window_start = self.start_time
        diagnostic.window_stop = rospy.get_rostime()
        diagnostic.samples = len(self.cpu_load_log)
	if len(self.cpu_load_log) > 0:
		...
		loads = cpuload()
		loads.cpu_load = cpu_load_log[cpu]
		diagnostic.cpu_load.append(loads)
	diagnostic.phymem_used = self.phymem_used_log
	diagnostic.phymem_avail = self.phymem_avail_log
	return diagnostic
