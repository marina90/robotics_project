...
from ros_diagnostic_msgs.msg import NodeDiagnostic
from ros_diagnostic_msgs.msg import HostDiagnostic
...
class Diagnostic(object):
    ...
    def __init__(self, sample_rate=None, update_rate=None):
        ...
        self.update_rate = update_rate or rospy.Duration(0.5)
        ...
        rospy.init_node('rosdiagnostic_%s' % nodename)
        ...
        self._node_publisher = rospy.Publisher('node_diagnostic', NodeDiagnostic, queue_size=10)
        self._host_publisher = rospy.Publisher('host_diagnostic', HostDiagnostic, queue_size=10)
    ...
    def _update_node_list(self, no_nodes = None):
        """ Contacts the master using xmlrpc to determine what processes to watch """
        def contains(node):
	  nodes = ["rviz", "rqt", "diagnostic", "rostopic", "rosnode", "record", "ATTACK", "attack"]
	  for n in nodes:
	    if n in node:
	      return True
	  return False
        nodenames = rosnode.get_nodes_by_machine(rosgraph.network.get_host_name())
        nodenames = filter(lambda node: not contains(node), nodenames)
        #if only_nodes != []:
	  #print "hi"
	  #nodenames = filter(lambda node: node in only_nodes, nodenames)
        #print nodenames
    ...
    def _publish_data(self, event=None):
        ...
        with self._lock:
            ...
            self._host_publisher.publish(self._host_monitor.get_diagnostic())
            self._host_monitor.reset()

            # Publish Each Node's statistics
            for node in self._nodes.values():
                self._node_publisher.publish(node.get_diagnostic())
                node.reset()
