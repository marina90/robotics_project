...
class NodeMonitor(object):
    ...
    def get_diagnostic(self):
        ...
        diagnostic.node = self.node
        diagnostic.host = self.hostname
        diagnostic.uri = self.uri
        diagnostic.pid = str(self.pid)
	diagnostic.samples = len(self.cpu_log)
        diagnostic.threads = self.num_threads
        diagnostic.window_start = self.start_time
        diagnostic.window_stop = rospy.get_rostime()
	diagnostic.cpu_loads = self.cpu_log
	diagnostic.virt_mems = self.virt_log
	diagnostic.real_mems = self.res_log
	return diagnostic
